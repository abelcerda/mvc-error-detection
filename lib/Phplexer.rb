...
class PhpLexer < Parslet::Parser
 ...
 #   rule(:php_code) 		{ dowhile_statement.as(:DOWHILE) >> blank }
		...
		rule(:php_code)         {(control_structure.as(:CONTROL_STRUCTURE) | pdo_statement | class_definition.as(:CLASS) | coment.as(:COMENT) |
...
#---------------- EXCEPTION -----------------------
		...
		rule(:tryCatch)       	{(str("try") >> blank >> str("{") >> blank >> ((php_close >> blank >> (tryContPhp | tryContPhpInc)) |
														(php_code.maybe >> blank >> str("}") >> blank >> (catch_php | catch_exc) >> blank >> finally_exc.maybe))) >> blank}
		rule(:tryContPhp)				{(content_ctrl_struct.repeat(1) >> blank >> ((php_open >> blank >> str("}") >> blank >> (catch_php | catch_exc)) | (php_open >> blank >>
														php_code.repeat >> blank >> str("}") >> blank >> catch_php))) >> blank }#Falta como sigue hacia el catch
		...
		rule(:catch_php)				{(str("catch") >> blank >> str("(") >> blank >> catch_conditions >> blank >> str(")") >> blank >> str("{") >>
														blank >> ((php_close >> blank >> (catchContPhp | catchContPhpInc)) |
														(php_code.maybe >> blank >> str("}") >> blank >> (finally_php | finally_exc).maybe))) >> blank }
		rule(:catchContPhp)			{(content_ctrl_struct.repeat(1) >> blank >> ((php_open >> blank >> str("}") >> blank >> (php_close | (finally_php))) | (php_open >> blank >>
														php_code.repeat >> blank >> str("}") >> blank >> (php_close | finally_php | finally_exc)))) >> blank  }
		rule(:catchContPhpInc)	{(php_open >> blank >> php_code.repeat >>str("}") >> blank >> finally_exc.maybe) >> blank  }

		rule(:finally_php)			{((str("finally") >> blank >> str("{")) >> blank >> (php_close >> content_ctrl_struct.repeat >>
														(php_open >> ((tryCatch_end) | (php_code.repeat >> blank >> str("}") >> blank >> php_close)))) ) >> blank }

		rule(:catch_exc)				{(str("catch") >> blank >> str("(") >> blank >> catch_conditions >> blank >> str(")") >> blank >>
														str("{") >> blank >> php_code.maybe >> blank >> str("}") >> blank) >> blank}
    ...
    rule(:thrown)           { ((php_open >> blank >> str("throw") >> blank >> str("new") >> blank >> internal_function >> str(";").maybe >> blank >>
														php_close) | (str("throw") >> blank >> str("new") >> blank >> internal_function >> str(";").maybe)) >> blank}
#--------------------------------------------------

#-------------------- Functions -------------------
    rule(:function_statement){ (str("parent::__construct") | simple_string) >> str("(") >> blank >> parameters.maybe >> blank >> str(")") >> blank >>
                            str(";").maybe >> blank >> coment.as(:COMENT_FUNCTION).maybe >> blank }
    rule(:define_function)  { str("function") >> blank >> (str("parent::__construct") | simple_string) >> blank >> str("(") >> blank >>
                            parameters.maybe >> blank >> str(")").maybe >> blank >>
                            coment.as(:COMENT_FUNCTION).maybe >> blank >> str("{") >> blank >> ((php_close >> blank >>
														content_ctrl_struct.repeat >> blank >> ((php_open >> blank >> php_code >> blank >> str("}") >>
														blank >> php_close)| function_end)) | (php_code >> blank >> str("}"))) >> blank}
...
#------------------- Clases -----------------------
    rule(:class_definition) { (str("abstract").maybe >> blank >> str("class") >> blank >> simple_string >> blank >>
                            ...
                            class_content.repeat >> blank >> (str("}").as(:normalENd) | class_end_php_tag | class_end )) >> blank }
    ...
    rule(:vars_definition)  { (type_var_method.repeat.maybe >> blank >> str("var").maybe >> blank >> (var_assignment.as(:VA) |
                            ((simple_string >> blank >> str(",") >> blank).repeat(1) >> simple_string) | simple_string) >> blank >>
                            str(";")) >> blank}
    ...
    rule(:method_definition){ (type_var_method.repeat(1) >> blank >> define_methods >> blank) }
#		rule(:alternative_cont)	{ (type_var_method.repeat.maybe >> blank >> define_methods >> blank) }
		rule(:define_methods)		{ (str("function") >> blank >> (simple_string) >> blank >> str("(") >> blank >>
                            parameters.maybe >> blank >> str(")").maybe >> blank >>
                            coment.as(:COMENT_FUNCTION).maybe >> blank >> str("{") >> blank >> ((php_close >> blank >>
														content_ctrl_struct.repeat.maybe >> blank >> ( function_end.as(:func_end) | (php_open >> blank >> php_code.repeat >> blank >> str("}") >>
														blank >> php_close).as(:end_phpClose) | (php_open >> blank >> php_code.repeat >> blank >> str("}") >>
														class_content.repeat).as(:end_without_phpClose))) | (php_code.maybe >> blank >> str("}")))) >> blank }
    ...
    rule(:class_instantiation){str("new") >> blank >> simple_string >> blank >> str("(") >> blank >>
														parameters.maybe >> blank >> str(")") >> blank}
...
#-------------------- Switch ----------------------
    ...
    rule(:swt_normal_syntax){str("switch") >> blank >> (operation | str("(") >> blank >> variable >> blank >>
                            ...
                            (switch_content | switch_case).repeat >> blank >> switch_default.maybe >> blank >> str("}") >> coment.as(:COMENT_SWITCH).maybe}
    rule(:swt_alternative_syntax){(str("switch") >> blank >> (operation | (str("(") >> blank >> variable >> blank >>
                            str(")"))) >> blank >> str(":") >> blank >> coment.as(:COMENT_SWITCH).maybe >> blank >>
														((php_close >> blank >> switch_content.repeat(1)) | switch_case.repeat(1)) >> blank >>
														((php_open >> str("endswitch;") >> blank >> php_close) | (str("endswitch;") >> blank >> php_close))) >> blank }
    ...
    rule(:switch_case_tphp)	{ (str("case") >> blank >> variable >> blank >> str(":") >> blank >> php_close >> content_ctrl_struct.repeat >>
		 												((php_open >> blank >> php_code.repeat(1) >> blank >> str("break;").maybe >> blank) | switch_alternative_end_php)) >> blank}
...
#---------------------- For -----------------------
    ...
    rule(:for_normal)       {(str("for") >> blank >> str("(") >> blank >> for_conditions.as(:CONDITIONS) >> blank >> str(")") >> blank >>
                            coment.as(:FOR_COMENTS).maybe >> blank >> str("{") >> blank >> ((php_close >> blank >> content_ctrl_struct.repeat >>
														blank >> (php_open >> blank >> (for_normal_close | (one_line_statement >> blank >> for_normal_close)))) |
														(php_code.maybe >> blank >> str("}")))) >> blank }
    rule(:for_alternative)  { (str("for") >> blank >> str("(") >> for_conditions >> str(")") >> blank >> str(":") >> blank >>
														((php_close >> blank >> content_ctrl_struct.repeat >>
														blank >> (php_open >> blank >> (for_alternative_close | (one_line_statement >> blank >> for_alternative_close)))) |
														(php_code.maybe >> blank >> str("endfor;")))) >> blank }
...
#------------------- Do While ---------------------
    rule(:dowhile_statement){(str("do") >> blank >> coment.as(:DO_WHILE_COMENT).maybe >> blank >> str("{") >> blank >>
                            ((php_close >> blank >> ((content_ctrl_struct).repeat >> ((php_open >> blank >> dowhile_close) | (php_open >>
														blank >> one_line_statement >> blank >> dowhile_close)))) | (php_code >> blank >> str("}") >> blank >>
														str("while") >> blank >> ((str("(") >> blank >> variable >> blank >> str(")")) | operation.as(:DO_WHILE_OPERATION)) >>
                            str(";")).as(:caminoNormal)) >> coment.as(:COMENT_DOWHILE).maybe) >> blank}
	  rule(:dowhile_close)		{(str("}") >> blank >> str("while") >> blank >> ((str("(") >> blank >>
														variable >> blank >> str(")")) | operation.as(:DO_WHILE_OPERATION)) >> blank >> str(";") >> blank >> php_close) >> blank}
#--------------------------------------------------
...
#------------------- While ------------------------
    ...
    rule(:while_normal_syntax){ str("while") >> blank >> expresions.as(:WHILE_OPERATION) >> blank >>
                            coment.as(:WHILE_COMENT).maybe >> blank >> str("{") >> blank >> ((php_close >> blank >> (content_ctrl_struct).repeat >>
														blank >> (php_open >> blank >> ((one_line_statement >> blank >> while_normal_end) | while_normal_end))) |
														(php_code >> blank >> str("}"))) >> blank}
    rule(:while_alternative_syntax) { str("while") >> blank >> expresions.as(:WHILE_OPERATION) >> blank >>
                            coment.as(:WHILE_COMENT).maybe >> blank >> str(":") >> blank >> ((php_close >> blank >> (content_ctrl_struct).repeat >>
														blank >> (php_open >> blank >> ((one_line_statement >> blank >> while_alternative_end) | while_alternative_end))) |
														(php_code >> blank >> str("endwhile;"))) >> blank }
...
#------------------- if ---------------------------
    rule(:if_statement)     {(if_one_line.as(:IF_ONE_LINE) | if_normal_form.as(:NORMAL_FORM) |
                            if_short_form.as(:SHORT_FORM)) >> blank}

		rule(:if_one_line)      {(str("if") >> blank >> operation >> blank >> str(")").repeat.maybe
                            coment.as(:IF_COMENT).maybe >> blank >> str("{") )}
		rule(:else_one_line)    { str("else") >> blank >> only_sentence >> blank }

		rule(:if_normal_form)		{(str("if") >> blank >> operation.as(:IF_OPERATIONS) >> blank >> str("{") >> blank >>
														((php_close >> blank >> content_ctrl_struct.repeat >> blank >> php_open >> blank >> php_code.maybe >>
														blank >> ((str("}") >> blank >> (else_tag_php_normal | else_normal_form)) | (str("}") >> blank >> php_close) |
														(str("}")) | ((if_elseIf_normal | ((str("elseif") | str("else if")) >> blank >> operation.as(:OPERATION_ELSE) >> blank >>
														str("{") >> blank >> php_code >> blank >> str("}") >> blank >> else_normal_form.maybe >> blank) |
														else_normal_form | else_one_line)).repeat.maybe)).as(:Php_INCOMPLETE_TAG) | (php_code.maybe >> blank >>
														str("}") >> blank >> coment.as(:IF_COMENT).maybe >> blank >>
														((if_elseIf_normal) | ((str("elseif") | str("else if")) >> blank >> operation.as(:OPERATION_ELSE) >> blank >>
														str("{") >> blank >> php_code >> blank >> str("}") >> blank >> else_normal_form.maybe >> blank) |
														(else_tag_php_normal | else_normal_form) | else_one_line).repeat.maybe).as(:NORMAL_WAY_IF))) >> blank}

		rule(:if_short_form)		{(str("if") >> blank >> operation.as(:IF_OPERATIONS) >> blank >> str(":") >> blank >>
														((php_close >> blank >> content_ctrl_struct.repeat >> blank >> php_open >> blank >> php_code.maybe >>
														blank >> (((else_tag_php_short | else_short_form)) | (str("endif;") >> blank >> php_close) |
														(str("endif;")) | ((if_elseIf_short | ((str("elseif") | str("else if")) >> blank >> operation.as(:OPERATION_ELSE) >> blank >>
														str(":") >> blank >> php_code >> blank >> str("endif;") >> blank >> else_short_form.maybe >> blank) |
														else_short_form | else_one_line)).repeat.maybe)).as(:Php_INCOMPLETE_TAG_SHORT) | (php_code.maybe >> blank >>
														coment.as(:IF_COMENT).maybe >> blank >>
														( str("endif;") | (((if_elseIf_short) | ((str("elseif") | str("else if")) >> blank >> operation.as(:OPERATION_ELSE) >> blank >>
														str(":") >> blank >> php_code >> blank >> str("endif;") >> blank >> else_short_form.maybe >> blank) |
														(else_tag_php_short | else_short_form) | else_one_line).repeat.maybe))).as(:NORMAL_WAY_IF_SHORT))) >> blank}


    rule(:if_elseIf_normal)	{(str("elseif") | str("else if")) >> blank >> operation >>
														blank >> str("{") >> blank >> php_close >> blank >> (content_ctrl_struct).repeat >> blank >>
														((php_open >> (one_line_statement | control_structure).repeat >> blank >> ((str("}") >> blank >> else_tag_php_normal) |
														(str("}") >> blank >> (php_close | eof)))) | ((php_open >> blank >> str("}") >> blank >> else_tag_php_normal) |
														if_normal_end)) >> blank }
		rule(:if_elseIf_short)	{(str("elseif") | str("else if")) >> blank >> operation >>
														blank >> str(":") >> blank >> php_close >> blank >> (content_ctrl_struct).repeat >> blank >>
														((php_open >> (one_line_statement | control_structure).repeat >> blank >> ((else_tag_php_short) |
														(str("endif;") >> blank >> (php_close | eof)))) | ((php_open >> blank >> str("endif;") >> blank >> else_tag_php_short) |
														if_short_end)) >> blank}
    rule(:else_tag_php_short){(str("else") >> blank >> str(":") >> blank >> php_close >> blank >>
													  (content_ctrl_struct).repeat >> blank >> (if_short_end | (php_open >> blank >> (one_line_statement |
														control_structure).repeat >> blank >> str("endif;") >> blank >> (php_close | eof))) >> blank)}
		rule(:else_tag_php_normal){(str("else") >> blank >> str("{") >> blank >> php_close >> blank >>
													  (content_ctrl_struct).repeat >> blank >> (if_normal_end | (php_open >> blank >> (one_line_statement |
														control_structure).repeat >> blank >> str("}") >> blank >> (php_close | eof))) >> blank) }
    rule(:else_normal_form) { str("else") >> blank >> coment.as(:ELSE_COMENT).maybe >> blank >> str("{") >>
                            blank >> ((php_close >> blank >> (content_ctrl_struct).repeat >>
														blank >> if_normal_end) | (php_code >> blank >> str("}"))) >> blank }
    rule(:else_short_form)  { (str("else") >> blank >> str(":") >> blank >> ((php_close >> blank >> (content_ctrl_struct).repeat >> blank) |
														php_code) >> blank >> (str("endif;") | if_short_end) >> blank >> coment.as(:COMENT_IF).maybe) >> blank }
...
#-------------------- ForEach ---------------------
    rule(:foreach_statement){(feach_normal_syntax.as(:FOREACH_NORMAL_SYNTAX) |
		...
		rule(:feach_alternative_syntax){ str("foreach") >> blank >> str("(") >> blank >> (var_array.as(:ARRAY) | variable) >> blank >>
														str("as") >> blank >> value_foreach >> blank >> str(")") >> blank >> str(":") >> blank >>
														((php_close >> blank >> (content_ctrl_struct).repeat >> blank >> (php_open >> ((one_line_statement >> blank >>
														foreach_alternative_end) | foreach_alternative_end))) | (php_code >> blank >> str("endforeach;"))) >> blank }

    rule(:feach_normal_syntax){ str("foreach") >> blank >> str("(") >> blank >> (var_array.as(:ARRAY) | variable) >> blank >>
														str("as") >> blank >> value_foreach >> blank >> str(")") >> blank >> str("{") >> blank >>
														((php_close >> blank >> (content_ctrl_struct).repeat >> blank >> (php_open >> ((one_line_statement >> blank >>
														foreach_normal_end) | foreach_normal_end))) | (php_code >> blank >> str("}"))) >> blank  }
    ...
    rule(:array_multiple_positions){ (str("array") >> blank >> str("(") >> blank >> (array_content | variable) >> blank >>
    												str(")")) >> blank }
    rule(:array_one_position){ ((str("$_post").as(:POST) | str("$_get").as(:GET) | str("array") | parent_string | simple_string) >> (str("[") >> blank >>
    ...
    rule(:asociative_array) { ((class_atributte | cadenas | simple_string) >> blank >> str("=>") >> blank >> (class_atributte |
                            ...
                            blank >> str("=>") >> blank >> (var_array | internal_function | operation | variable)) |
                            (var_array | internal_function | operation | variable)) >> blank }
...
#--------------------------------------------------
    ...
    rule(:rigth_part)       { (dba_statement.as(:DBA_STATEMENT) | pdo_statement.as(:PDO_STATEMENT) | operation.as(:OPERATION) | var_array.as(:ARRAY) |
                            ...
                            internal_function.as(:FUNCTIONS) |  variable) >> blank }
    ...
    rule(:expresions)       {(((str("(").repeat.maybe >> blank >> (internal_function | array_one_position | param_class |
                            ...
                            param_class | variable) >> blank >> str(")").repeat).as(:OP1) | (((internal_function | array_one_position |
														param_class |variable) >> blank >> operators).repeat(1) >> blank >> (internal_function | array_one_position |
														param_class |variable)).as(:OP2)) >> blank }
    rule(:variable)         { ( class_atributte | cadenas | array_one_position | internal_function | parent_string |
    ...
    rule(:internal_function){ ((str("parent::__construct") | dba_statement.as(:DBA_STATEMENT) | parent_string | simple_string) >> blank >>
														str("(") >> blank >> parameters.maybe >> blank >> str(")").maybe) >> blank }
    ...
    rule(:only_argument)		{ (str("(") >> blank >> (array_one_position | internal_function | class_atributte | cadenas |
                            variable) >> blank >> str(")")) >> blank }

    rule(:parameters)       {((operation | class_atributte | var_array.as(:ARRAY_PARAM) | internal_function.as(:INT_FUNC_PARAM) |
                            param_class.as(:PrmCL) | variable) >> blank >> str(",") >> blank).repeat.maybe >>
                            blank >> (operation | class_atributte | var_array.as(:ARRAY_PARAM) | internal_function.as(:INT_FUNC_PARAM) |
...
class PhpTransformer < Parslet::Transform
    ...
    rule(:GET => simple(:x)) {
        ...
        resp = {}
        resp[:HTML_SECTION]= x.line_and_column      #con esto se puede obtener la ubicación del token en la cadena de entrada
        resp
    }
