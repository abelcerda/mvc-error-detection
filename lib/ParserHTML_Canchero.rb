...
class ScriptParser < Parslet::Parser
    ...
    rule(:script_tag_section)   { script_tag_open >> noscript >> script_tag_close }
    rule(:script_tag_open)	    { (str('<script') >> blank >> script_attribute? >> str('>')) >> blank }    
    rule(:script_attribute?)    { script_attribute.repeat(1).maybe }    
    rule(:script_attribute)     { script_attr_type >> equal >> cuote >> script_attr_value >> cuote >> blank }    #hay que controlar que si se abren comillas simples los valores deben aceptar las comillas dobles y viceversa
...
class Transformer < Parslet::Transform
    ...
    rule(:SCRIPT_EMBEDDED => simple(:x)) { 
        res = {}
        #res[:hola]= String(x)   #+line
        #res[:chau]= String(y)+"qw"  #+col   #con esto se convierte el valor del token a cadena
        res[:SCRIPT_EMBEBIDO]= x.line_and_column      #con esto se puede obtener la ubicación del token en la cadena de entrada
        #arr = []
        #arr << [:hola=> String(x)]
        #arr << [:chau=> String(y)]
        #res[:arr] = arr
