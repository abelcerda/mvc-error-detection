...
class ScriptParser < Parslet::Parser
    ...
    rule(:script_attr_type)     { (str('src').as(:SRC) | anyattribute) >> blank }   #agregar la regla de atributos personalizados de :attributes    
...
class Mini < Parslet::Parser
    ...
    rule(:content)              { ( comment | script | tag_section | text ).repeat }
...
class Transformer < Parslet::Transform
    ...
    rule(:SCRIPT_INLINE => simple(:x)) { 
        ...
        res[:SCRIPT_EN_LINEA]= x.line_and_column      #con esto se puede obtener la ubicación del token en la cadena de entrada
        #arr = []
        #arr << [:hola=> String(x)]
        #arr << [:chau=> String(y)]
        #res[:arr] = arr
...
#    }
    ...
    rule(:SCRIPT_EMBEDDED => subtree(:x)) { # DOESN'T WORK
        res = {}
        if (x.size)
            i=0
            innocent = true
#            while ((i < x.size) && innocent)
#                if(key.to_s=="SCR_VAL")
#                    if (val.to_s == "text/html")
#                        innocent = false
#                    end
#                end
#            x.each do |key,value|
#            for element in x
#                if innocent
#                if ((defined? element[:SCR_VAL]))
##                    if ((element[:SCR_VAL].to_s.size))
##                        if (element[:SCR_VAL].to_s === "text/html")
##                            innocent = false
##                        end
##                    end
##                end
#                    res[i]= element[:SRC_VAL].to_s
##                    end
#                else
#                if((key.to_s=="NOSCR"))
#                    res[:CODE]= value.to_s
#                        
#                else
#                    if((defined? value[:SCR]))
#                        res[i]= value[:SRC].to_s
#                    end
#                end
#                end
#                res[i]= key
...
#                res[:code] = "exito"
#            end
#            if (x.find_index("text/html"))
#                res[0] = "found"
            i=0
            y={}
            #            x.each do |key|    #WORKS!! key = {:SCR_VAL => " ... "} and siblings
#                if (defined? key[:SCR_VAL].size)
#                    if (key[:SCR_VAL].to_s == "text/javascript")
#                        y[i]=key
#                    else
#                        y[i]="NONONO"
#                    end
...
#                end
#            end
            if (defined? x[0][:SCR_VAL].size)
                if (x[0][:SCR_VAL].to_s == "text/javascript")
                    y=x
...
#                res[0] = y.find_index("text/javascript")
#            end
            res=y
        end
        res
...
#    rule(:SCR_VAL => simple(:v), :NOSCR => simple(:script)) { #USELESS
#        res = {}
#        if (v=="text/html")
#            res[:type]="HTML"
#            res[:code]=script
